...
describe('fetch-art MAP ↔ deck img consistency', () => {
  it('parses a non-trivial MAP out of scripts/fetch-art.mjs (anti-drift guard)', () => {
    // 158 entries at v1.7 (up from ~145 at v1.4, plus the Long Vowels deck's
    // MAP entries); a collapsed parse means the regex or the MAP moved — fail
    // loudly instead of passing on an empty key list.
