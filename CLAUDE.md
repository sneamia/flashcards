...
- **In flight: v1.7.0** on branch `v1.7/magic-e-category` — NOT yet reviewed, PR'd, merged or deployed. Adds the **Long Vowels** category (id `magic-e`, title differs from the id on purpose) holding one 28-card **Magic E** deck; 8 words (snake, grape, plate, plane, skate, whale, slide, flute) are deliberate cross-category duplicates of blends/`wh` cards, the app's first. App totals: **18 decks / 210 cards / 202 distinct words**, pools 70/55/57/28, **art 173/210 (82%)**, 165 art SVGs, 158 MAP entries, **101 unit / 35 e2e**. All four gates green. Deliberate: the deck carries **no `graphemes` key** (only deck of 18 that doesn't) — see the TODOS convention item before "fixing" that.
