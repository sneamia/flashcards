...
- **In flight: v1.7.0** on branch `v1.7/magic-e-category` — NOT yet reviewed, PR'd, merged or deployed. Adds the **Long Vowels** category (id `magic-e`, title differs from the id on purpose) holding one 28-card **Magic E** deck; 8 words (snake, grape, plate, plane, skate, whale, slide, flute) are deliberate cross-category duplicates of blends/`wh` cards, the app's first. App totals: **18 decks / 210 cards / 202 distinct words**, pools 70/55/57/28, **art 171/210 (81%)**, 163 art SVGs, 156 MAP entries, **105 unit / 36 e2e**. All four gates green. Deliberate: the deck carries **no `graphemes` key** (only deck of 18 that doesn't) — see the TODOS convention item before "fixing" that.
...
- **Active branch:** `v1.7/magic-e-category` (16 commits, unmerged — 10 implementation/chore + 6 docs). The v1.6 record it replaced in `handoff.md` lives at git `1783040:handoff.md`; the v1.3 predecessor at git `2985084:handoff.md`.
